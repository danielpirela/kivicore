                ...
                <div className='fixed top-0 left-0 overflow-hidden bg-none opacity-95 min-h-screen min-w-full flex justify-center items-start'>
                    <div className='bg-slate-100 rounded-lg p-3 ring-2 ring-red-600 mt-4'>
                        <h1 className=' text-lg'>
                            Estas seguro que quieres borrarla?
                        </h1>
                        <div className='space-x-1 justify-center items-center flex py-2'>
                            ...
                            <button
                                ...
                                className='px-4 py-2 bg-red-600 text-center text-white rounded-lg w-32'
                            >
                                Cancelar
                            </button>
                        </div>
            ...
            {citas &&
                citas.map((cita, index) => {
                    return (
                        <div
                            key={cita.id}
                            className='gap-4 flex m-2 max-[639px]:justify-center max-[639px]:items-center'
                        >
                            <ul
                                className='shadow-md flex flex-col gap-2 p-2 items-center md:flex-row rounded-md ring-2 ring-indigo-600 w-auto justify-start sm:flex-row'
                                key={cita.id}
                            >
                                {pacientes.map(paciente => {
                                    let tpm = ''
                                    const a = paciente.appointment.find(
                                        appointment =>
                                ...
                                <li className='justify-center items-center flex'>
                                    <button
                                        className={
                                            'rounded-lg  p-1 text-white bg-indigo-500 justify-center items-center flex'
                                        }
                                        onClick={e => {
                                            handleEdit(e, index)
                                        }}
                                ...
                                <li>
                                    <button
                                        className={
                                            'rounded-lg p-1 text-white bg-red-500 justify-center items-center flex'
                                        }
                                        onClick={e => {
