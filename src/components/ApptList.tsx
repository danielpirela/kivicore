    ...
    return (
        <div>
            ...
            {isEditing && (
                <>
                    <button
                        className='p-1 bg-indigo-600 absolute right-0 top-0 mt-16 mr-2 rounded-lg z-50'
                        onClick={() => {
                            setIsEditing(!isEditing)
                        }}
                    >
            ...
            {citas &&
                citas.map((cita, index) => {
                    return (
                        <div
                            key={cita.id}
                            className='gap-4 flex m-2 max-[639px]:justify-center max-[639px]:items-center animate-fade-left animate-delay-150'
                        >
                            <ul className='shadow-md flex flex-col gap-2 p-2 items-center md:flex-row rounded-md ring-2 ring-indigo-600 w-auto justify-start sm:flex-row'>
                                ...
                                <li>
                                    <button
                                        className={
                                            'rounded-xl px-1 py-1 text-white bg-blue-500'
                                        }
                                        onClick={e => {
                                            handleEdit(e, index)
                                        }}
                                ...
                                <li>
                                    <button
                                        className={
                                            'rounded-lg px-1 py-1 text-white bg-red-500 '
                                        }
                                        onClick={e => {
