...
export function CitasAdminList ({citas, pacientes} : Props){
    ...
    const handleChecking = (e:React.MouseEvent<HTMLElement> , index: number) =>{
        e.preventDefault()
        setCitaIndividual(citas[index])
    ...
    return (
        <div className=" relative overflow-auto mt-4 shadow-lg animate-fade-left h-auto">
                ...
                <div className='absolute top-0 left-0 overflow-hidden bg-none opacity-95 min-h-screen min-w-full flex justify-center items-start'>
                    <div className='bg-white rounded-lg p-2 ring-2 ring-red-600 mt-4 shadow-2xl animate-fade-down delay-150'>
                        <div className='space-x-1 justify-center items-center flex py-1 px-2'>
                            ...
                            <button onClick={() => {setDeleteView(!deleteView)}} className='p-2 bg-red-600 text-center text-white rounded-lg'>
                                <IconClose w={24} h={24}/>
                            </button>
                        </div>
                    </div>
...

            {
                isEditing && (
                    <>
                        <button className='p-1 bg-indigo-600 fixed right-0 top-0 mt-2 mr-2 rounded-lg z-50' onClick={()=>{setIsEditing(!isEditing)}}>
                            <IconClose w={24} h={24}/>
                        </button>
            ...
            {   isChecking &&(
                <>
                    <button className='absolute right-0 top-0 mr-4 mt-20 text-indigo-600 z-20 text-2xl bg-indigo-700 p-1 rounded-lg ' onClick={() => {setIsChecking(!isChecking)}}>
                        <IconClose w={24} h={24}/>
                    </button>
                    <ModalEditMedicoCita>
                ...
                <tbody>
                    {
                        citas.map((cita, index) =>{
                            return (
                                <tr className="bg-white border-b hover:bg-slate-200" key={cita.id}>
                                    ...
                                    <td className='px-4 py-3 flex justify-start items-center'>
                                        <p className={`rounded-lg px-2 py-1 text-center
                                    ...
                                    <td className='px-5 py-3 flex-col justify-start items-center'>
                                        <button className='bg-indigo-500 rounded-md p-1 mr-2'
