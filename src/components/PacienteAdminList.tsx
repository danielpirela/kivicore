            ...
            <table className='w-full text-sm text-left rtl:text-right text-gray-800 table-auto'>
                <thead className='text-xs text-gray-700 uppercase bg-gray-50'>
                    <tr>
                        ...
                        <th className='px-6 py-3 tracking-wider'>Telefono</th>
                        <th className='px-6 py-3 tracking-wider'>Genero</th>
                        <th className='px-6 py-3 tracking-wider'>Correo</th>
                    ...
                    {pacientes.map((paciente, index) => {
                        return (
                            ...
                            >
                                ...
                                <td className='px-6 py-4'>{paciente.gender}</td>
                                <td className='px-6 py-4'>{paciente.email}</td>
                                <td className='px-4 py-3 flex justify-start items-center'>
                                    <p
                                        className={`px-2 py-1 text-center rounded-lg ${paciente.status === 'Asegurado' ? 'bg-green-500' : 'bg-red-500'}`}
                                    >
                                        {paciente.status}
                                    </p>
                                </td>
